...
| `MQTT command <name> ignored: mqtt.allow_write_entities is on but the broker connection is anonymous — set mqtt.username/password (broker ACLs are the only authentication a publish carries)` | **3.80.0:** the broker session must be authenticated | Add credentials to the broker (`mosquitto/config/mosquitto.conf` — two lines + `mosquitto_passwd`) and set `mqtt.username`/`password` in the gateway |
